...
This repository builds four native applications:

- `touch_ray_demo`: the multi-touch tracker.
- `touch_canvas`: a touch drawing canvas with drawing export and screenshot buttons.
- `touch_accuracy`: a 100 px grid target test that measures touch accuracy.
...
`dist/linux-musl/touch_canvas`, `dist/linux-musl/touch_accuracy`, and
...
`dist/linux-glibc/touch_canvas`, `dist/linux-glibc/touch_accuracy`, and
...
`dist\windows\touch_canvas.exe`, `dist\windows\touch_accuracy.exe`, and
